...
class SubforumAdapter(val onClick: (thread: Thread) -> Unit) : RecyclerView.Adapter<SubforumAdapter.SubforumHolder>() {
    ...
    class SubforumHolder(val view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(thread: Thread) {
            if (thread.isSticky) {
                view.threadSticky.visibility = View.VISIBLE
            } else {
                view.threadSticky.visibility = View.GONE
            }

            view.threadTitle.text = thread.title
            view.threadPages.text = view.context.getString(R.string.thread_pages, thread.pages)
        }
    }
}