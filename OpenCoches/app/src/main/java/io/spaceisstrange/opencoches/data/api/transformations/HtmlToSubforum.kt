...
/**
 * Clase que transforma un documento en el contenido de un subforo.
 */
class HtmlToSubforum {
    companion object {
        ...
        fun transform(document: Document): List<Thread> {
            ...
            try {
                val pinnedThreadsBody = document.select("tbody[id^=${ApiConstants.THREAD_STICKY_KEY}]")[0]
            ...
            val nonPinnedThreadsBody = document.select("tbody[id^=${ApiConstants.THREAD_NO_STICKY_KEY}]")[0]
